...
街灯は絵が縦 2 マスでも、通行不可は灯のある上の 1 マス(構造物の `cell` そのもの)だけで、
下の柱の根元は歩いて通れる。主人公が街灯の描かれたマスに立つと、主人公の上へ半透明の街灯を
1 組だけ重ね描きして透けて見せる。重なりの判定は `src/lib/village/lamp-veil.ts` の純粋関数
`lampVeilAt()`、重ね描きする層は `src/components/VillagePage/components/Village/components/LampVeil/`。
...
投げてから巻物が跳ねるまでは、竿の先から浮きまで糸をドット絵で渡す(竿を振っている間は隠す)。地点ではないので `Spot` を持たず、
規則は `src/lib/village/fishing.ts`、文言は `text.fishing` が持つ。釣果はプロフィールの
...
7. `Spot.action` — **会話窓ではなく専用の窓を開く地点のときだけ**。その地点は `text.stops` を持たないので、
   `src/lib/content/validate-world.ts` の文言検査から除く(卓上時計はこの手順で足した)
