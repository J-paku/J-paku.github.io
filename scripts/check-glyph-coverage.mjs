...
// 広い幅だけで検査すると、その文言が描画すらされず検査対象から漏れる。漏れた文字が
// システムフォントへ落ちていても検査は気づけない(PASS 方向に間違う壊れ方 — 03-pitfalls #5・#7 と同型)
const VIEWPORTS = [
...
for (const targetPath of targetPaths) {
  ...
  // scans には両方の幅のスキャン結果を集め、後段でまとめて幅を跨いだ合集合にする
  const scans = []
  ...
  for (const viewport of VIEWPORTS) {
    ...
    // 畳み状態の文言はここでしか拾えない
    const viewportScans = [await scanGlyphs()]
    ...
    for (const toggle of detailToggles) {
      ...
      viewportScans.push(await scanGlyphs())
    }
    ...
    // この幅で採ったスキャン列に幅ラベルを付け、幅を跨いだ合集合(scans)へ積む
    for (const scan of viewportScans) {
      scans.push({ ...scan, viewport: viewport.label })
    }
    await page.close()
  }

  // 全パス×全幅のスキャンを文字+フォントチェーン単位の合集合にする。同じ組み合わせが複数の状態に
  ...
  for (const scan of scans) {
    for (const item of scan.failures) {
      ...
      // 同じ失敗が複数の幅にまたがることもあるため、検出された幅ラベルを集合で持つ
      const viewportsSeen = failuresByKey.get(key)?.viewports ?? new Set()
      viewportsSeen.add(scan.viewport)
      failuresByKey.set(key, { ...item, viewports: viewportsSeen })
    }
  }
  ...
  if (inspected === 0) {
    ...
    console.error(`[FAIL] ${targetPath}: 検査対象のCJK/ハングルが0件(wide+narrow)。描画前に判定した可能性がある`)
    totalFailures += 1
    continue
  }

  if (result.failures.length === 0) {
    console.log(`[OK] ${targetPath}: 対象 ${inspected}種すべて自己ホスト(${usedFonts.join(' / ')})[wide+narrow]`)
  } else {
    console.log(
    ...
    for (const item of result.failures) {
      ...
      const viewportLabel = [...item.viewports].join('+')
      console.log(`  '${item.char}' w${item.weight} @ ${item.at} [${viewportLabel}] — ${reason}`)
    }
  }
}
