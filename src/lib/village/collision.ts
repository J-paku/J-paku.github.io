...
// 縦の経歴碑(stele)は縦 2、街灯は cell の 1 マス下、その他は 1×1
export const structureRect = (structure: Structure): Rect => {
  ...
  // 1×1 でも既定に任せられない唯一の例外。絵の cell が指すのは上のマスの灯で、
  // 塞ぐのは柱が地面に立つ真下のマス(灯笠の裏にあたる cell のマスは歩いて通れる)
  if (structure.kind === 'lamp') return { x, y: y + 1, w: 1, h: 1 }
  return { x, y, w: 1, h: 1 }
}
