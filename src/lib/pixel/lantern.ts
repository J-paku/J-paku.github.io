// 夜に灯る手提げランタンの正本。主人公(actors.ts)もロボット(structures.ts)も、
// ランタンの絵と重ね方はここだけから取る。2 箇所に描くと「同じ道具」のはずが、
// 片方だけ描き直されて黙って別物になる
//
...
//  - 主人公の足元の行(18)へ下ろさない。地面に置いた物に見え、持ち物として読めなくなる。
//    これは 16×24 の主人公だけの掟で、16×16 のマス 1 枚に収まるロボットには当てはまらない
//    (ロボットは受け皿が最終行に載る — 高さの根拠は structures.ts の ROBOT_LANTERN_TOP に書いた)
...
// 「主人公とロボットが同じ 1 枚を提げているか」を外から機械で確かめられるように公開する。
// この 2 文字がランタンの持つ灯り用文字のすべてで、どちらも palette-phase.ts の予約文字
