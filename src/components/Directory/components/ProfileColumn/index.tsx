...
//
// v2 ではサーバコンポーネント。状態を持たず、詳細への導線は同一ページ内の
// CareerDetail へのアンカーに置き換えた(JS 無しでも辿れる)
import type { Locale, Profile, UiStrings } from '@content/types/content'
...
function ProfileColumn({ profile, ui, locale }: ProfileColumnProps) {
  return (
    <div className={styles.column}>
      ...
      <div className={styles.top}>
        ...
        <p className={styles.role}>
          <PhraseText text={profile.role} locale={locale} />
        </p>
        <p className={styles.scope}>
          <PhraseText text={profile.scope.join(SEPARATOR)} locale={locale} />
        </p>
        <p className={styles.claim}>
          <PhraseText text={profile.headline} locale={locale} />
        </p>

        <ul className={styles.cv}>
          {profile.careers.map(career => (
            <li key={career.id} className={styles.cvItem}>
              <p className={styles.when}>{career.period}</p>
              <p className={styles.org}>
                <PhraseText text={career.company} locale={locale} />
              </p>
              <p className={styles.post}>
                <PhraseText text={career.role} locale={locale} />
              </p>
                ...
                <ul className={styles.assignments}>
                  {career.assignments.map(assignment => (
                    <li key={assignment.period} className={styles.assignment}>
                      ...
                      <PhraseText text={assignment.label} locale={locale} />
                    </li>
                  ))}
                </ul>
              ) : null}
              <p className={styles.tech}>{career.stack.join(SEPARATOR)}</p>
              {/* 詳細を持つ経歴だけが導線を出す。同一ページ下方の CareerDetail へのアンカー */}
              {career.detail !== undefined ? (
                <a className={styles.detailTrigger} href={`#career-${career.id}`}>
                  {/* 導線が3つ並ぶため、どの経歴のものかを読み上げ名だけに社名で足す。
                      表示文字列の合成はしない(社名も文言も content から来たものをそのまま置く) */}
                  <span className='sr-only'>{career.company}</span>
      ...
      <div className={styles.out}>
        <div className={styles.card}>
          <p className={styles.location}>
            <PhraseText text={profile.location} locale={locale} />
          </p>
          ...
          {profile.links.email !== undefined ? (
            ...
            <a
              ...
              className={`${styles.outLink} ${styles.mail}`}
            >
              {profile.links.email}
            </a>
          ) : null}
